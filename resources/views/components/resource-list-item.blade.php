...
<article class="group flex flex-col gap-5 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:border-indigo-200 hover:shadow-md sm:flex-row sm:items-center sm:justify-between">
    <div class="flex min-w-0 items-center gap-4">
        ...
        <div class="min-w-0">
            <h2 class="truncate text-base font-semibold text-slate-950">
                @if ($viewUrl)
                    <a href="{{ $viewUrl }}" class="transition hover:text-amber-700">{{ $title }}</a>
                @else
                    {{ $title }}
                @endif
            </h2>
    ...
    <div class="flex shrink-0 gap-2">
        @if ($viewUrl)
            <a href="{{ $viewUrl }}" class="inline-flex items-center justify-center rounded-xl bg-amber-50 px-4 py-2.5 text-sm font-semibold text-amber-700 transition hover:bg-amber-100">
                View
            </a>
        @endif
